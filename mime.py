...
import structured_learning_util as slu
...
def run_test(seqs, label_seqs, sess, preds_T, input_PHs, label_PHs, mask_PHs, seq_length_PH, loss_T, options):
    ...
    for idx in xrange(len(label_seqs) / batch_size):
        ...
        inputs, _, masks, seq_length = slu.st_preprocess_hf_aux(batch_x, options)
...
def train(
    ...
    #total_params = slu.count_parameters("rnn")
    ...
    train_seqs, train_labels, valid_seqs, valid_labels, test_seqs, test_labels = slu.load_data(
            options['input_path'],
    ...
    with tf.Session() as sess:
        ...
        for train_iter in xrange(options['num_iter']+1):
            batch_x, batch_y = slu.sample_batch(train_seqs, train_labels, options['batch_size'])
            inputs, labels, masks, seq_length = slu.st_preprocess_hf_aux(batch_x, options)
